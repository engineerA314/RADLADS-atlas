...
static void forward_exact_ckpt(
    ...
    TORCH_CHECK(S_ckpt.dtype() == torch::kFloat32 && Z_ckpt.dtype() == torch::kFloat32, "checkpoints must be fp32");
...
static void backward_exact_ckpt(
    ...
    cuda_backward_exact((int)BH, (int)T, (int)n_ckpt,
                        ...
                        S_T, Z_T, S_ckpt, Z_ckpt, S0, Z0,
                        dy, dS_T, dZ_T,
                        dphi_k, dphi_q, ddelta, dlr, ddecay, dbeta, dgate, dS0, dZ0);
}

// Debug backward: returns per-BH stats from kernel
