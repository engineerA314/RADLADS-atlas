...
def compile_atlas_omega(verbose=True):
    ...
    # Compilation flags (match atlas_omega_autograd.py defaults)
    dim_head = int(os.environ.get("ATLAS_CUDA_DIM_HEAD", "64") or 64)
    omega_window = int(os.environ.get("ATLAS_CUDA_OMEGA_WINDOW", "16") or 16)
    ...
    extra_cuda_cflags = [
        ...
        f"-D_C_={dim_head}",  # d dimension
        f"-D_E_={omega_window}",  # omega_window
        "--use_fast_math",
    ]
